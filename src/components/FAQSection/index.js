...
const FAQSection = () => {
	...
	return (
		<section className="py-20  bg-gradient-to-br from-greencustom via-greencustom2 to-greencustom3 drop-shadow-xl flex flex-col items-center">
			<div className="max-w-3xl text-center mb-12 drop-shadow-xl mx-4">
				<h2 className="text-3xl font-bold text-white dark:text-gray-700">FAQ</h2>
				<p className="text-gray-700 dark:text-black">
					Retrouvez ici les réponses aux questions les plus courantes.
				</p>
			</div>

			<div className="bg-white dark:bg-gray-700 shadow-lg rounded-3xl w-auto max-w-4xl p-10 px-4 mx-4">
				{faqData.map((item, index) => (
					<div key={index} className="border-b border-gray-300 mb-4 pb-4">
						...
						>
							<h2 className="text-lg sm:text-xl font-medium text-gray-900 dark:text-white m-5 mt-0 mb-0">{item.question}</h2>
							<span className={`mr-10 pr-5 w-5 h-1 bg-greencustom text-white dark:text-gray-900 rounded-full p-1 transition-transform ${activeIndex === index ? 'rotate-90' : ''}`}>
								<i className={`ri-add-line ${activeIndex === index ? 'ri-subtract-fill' : ''}`}></i>
							</span>
						</div>
						<div
...
const faqData = [
	...
	},
	{
		...
		answer: 'Utilisez des sites de compression d\'images en ligne, ou si vous préférez, vous pouvez envoyer un email à notre équipe de support technique avec les images en pièce jointe. Dans votre email, incluez des indications sur où et comment vous souhaitez que les images soient ajoutées à votre site web.',
	},
];
